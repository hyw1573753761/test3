...
static void  DES_Efun(char input[8], char key_in[8], int output[64])
{
	...
	CharToBit(key_in, key, 8);// 转换密文的二进制
	...
	F_func(r[15], l[16], subkeys[15]);//完成DES算法轮变换
	Xor(l[16], l[15], 32);//异或操作
	for (t = 0; t < 32; t++)
	{
		output_1[t] = l[16][t];
		output_1[32 + t] = r[16][t];
	}
	IP_In(output_1, output, IPR_Table);//逆IP
};
static void  DES_Dfun(int input[64], char key_in[8], char output[8])
{
	...
	CharToBit(key_in, key, 8);//把CHAR转换为INT
	subKey_fun(key, subkeys);//子密钥生成
	for (int i = 0; i < 32; i++)
	{
		l[0][i] = Ip[i];
